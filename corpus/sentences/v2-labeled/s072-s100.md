...
**Purpose:** Test Monday's hypothesis that "magic" in a Tonesu-style setting (space mecha + loose mystic system) does not require a new primitive — that `ra`, `wi`, `to`, `fe`, `ne`, and `se` already cover the conceptual ground. Seven sentences drawn from the domain. Verdict criterion: does the sentence come out clean, serviceable, or distorted? "Maintenance manual haunted by a ghost" = distorted.
...
**Verdict S076: clean and evocative.** `ti-mu` (relic) is immediately useful. `no-to-ge` correctly produces the Tonesu framing of "strange": not "weird" in an emotional sense, but "outside the current pattern-recognition schema." A properly creepy ancient artifact in Tonesu terms is one that emits signals that no existing `to` framework can classify.
...
**Verdict S077: clean.** `fe-no-ka` (forbidden boundary / prohibition threshold) is a natural compound. The Tonesu framing of "taboo" as a *boundary with a no-action property* is exactly right — prohibition is a structural condition, not a feeling.
...
**The mystic-primitive question is not closed, just deferred properly.** If corpus sentences keep appearing where the `wi`-as-instrument vs `wi`-as-direct-agent ambiguity (S075) matters — i.e., where Tonesu speakers would systematically need to distinguish trained-will-resonance from physical-command — and no compound patch resolves it, that is when a dedicated domain root becomes justified. The threshold: more than three attested sentences where the ambiguity produces genuinely wrong or misleading readings.
...
**Verdict S079: clean.** `wi-si` distinguishes prayer from command (`ka-si`) and from ordinary signal-emission. The Tonesu framing — prayer as will-signal, not petition-to-supernatural-authority — is consistent with an analytic epistemic culture.
...
- The Tonesu framing: a relic is sacred not because of divine decree but because it is an entity possessing recognized value that carries a boundary (approaches, uses, states of contact are restricted). This is structurally precise and culturally exactly right for an analytic culture.
...
*Tonesu framing: this is an epistemic claim within a doctrine, not an ontological assertion.*
...
**Verdict S084: serviceable.** The sentence works and produces the correct Tonesu framing: soul/spirit is a doctrine-level claim, not an uncontested ontological fact. `zo-to` and `zo-si` are both useful but need to be kept distinct. Logged as a watch item.
...
| S082 | priest / doctrine keeper | `to-su-li` | clean — same as librarian/archivist (Tonesu structural finding) |
