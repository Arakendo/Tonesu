...
*Ordinal function:* `[digit] ti` = `[n]`th in sequence. The `ti` (time/sequence root) in suffix position on a digit gives ordinal position. This is already specified in `notes/anchor-inventory.md`; here it is first attested in a sentence requiring it.
...
`hin pir nu pa` = 5 + `pir` (kilo-scale) + quantity-of space = 5 kilometers. Digit + scale + `nu` + domain = the full measurement stack from `notes/anchor-inventory.md` § Scale-prefixed measurement. First corpus attestation of the complete stack.
