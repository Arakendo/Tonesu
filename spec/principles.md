...
- **A substrate root is not a named-emotion root.** A primitive naming the raw felt-interior substrate of affective experience — internal signal before it resolves into any conceptual model — is categorically different from a primitive named for a specific emotion. `xa` (candidate, EM-002) would name the substrate, not the state: the same way `ma` names raw matter without naming any particular object. If added, `xa` may not be used as an emotion shorthand (`xa-vo-be` ≠ "joy"); process constructions still govern named states. The guard: `xa` appears only where the substrate itself is foregrounded — unresolved affect, numbness, observer-mode separation. See EM-002.
...
*Corollary: do not add sensory or embodied-experience primitives preemptively. Run the 20 ordinary-life sentence test (see notes/open-questions.md) first. If a concept fails to express without genuine distortion — not just verbosity — evaluate it as a primitive candidate. The specific test for a substrate primitive (`xa`): does `zo-se` (organism-perception) produce a structurally wrong reading for internal affective signal — not merely a verbose one?*
