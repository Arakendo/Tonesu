...
**Corpus notation use: clausal-scope display bracket.** In metalinguistic, corpus notation, and pedagogical contexts — where the surrounding text is Tonesu morphemes under analysis rather than running prose — `[]` additionally serves as a **clausal-scope display bracket**, marking the boundaries of grammatically essential constituents (complement clauses, object clauses, negation scope, relative clauses) for analytic legibility. In this use, removal invariance does not hold; the brackets are analytic scaffolding, not removable commentary. This use is register-distinct from the aside/commentary function: in corpus notation every token is a morpheme and `[]` is visibly structural; in Tonesu prose the aside reading is default. Genre and register make the mode unambiguous — the two uses never appear in the same context. The `Written:` stripping convention encodes this mechanically: `[]` is stripped from `Written:` output because it is display-only analytic scope; `{}` is preserved because it carries content-normative structural scope. Formal precedent: the `-` morpheme boundary mark applies the same principle — absent in Tonesu prose, normative in metalinguistic analysis.
...
**Audit date: March 2026. Basis: 32 roots + 6 frame particles + 3 discourse particles + 4 pronouns + 1 derivational suffix.**

Taken forms (46 of 80):

| Category | Forms |
|----------|-------|
| Roots (33) | `mu ma li zo ki ka be de su to fe ne pe go du pa di ti re se so lu si vo wi no nu ru pu ra ko ha fa` |
| Frame particles (6) | `la lo ro ta na da` |
| Discourse particles (3) | `he ya ke` (G029 vocative · G030 attention-signal · G032 pivot; CVP-001 / DIP-001, March 2026) |
...
The `f`-initial cluster (`fa fi fo fu`) and the `h`-initial cluster (`hi ho hu`) remain the primary reserve banks (`he` now taken — G029; 3 of 4 h-slots still free). Speech-speed risk: `ku` (four taken k-neighbours: `ka ki ko ke`); `ye`/`yi`/`yo` between taken `ya` and `yu`; `za`/`zi`/`zu` adjacent to taken `ze`/`zo`.
