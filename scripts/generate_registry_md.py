#!/usr/bin/env python3
"""
generate_registry_md.py  —  Pipeline Step 1

Reads registry/entries.yaml and regenerates the human-readable markdown files
in registry/derived/ that are used for GitHub browsing:
...
These files are the normative source record for registry entries (the structured
data in entries.yaml is derived from them via the one-time migration, and kept in
sync by accept_registry.py and direct edits). Run this step whenever entries.yaml
changes, to keep the GitHub-browsable markdown current.

The website build (Step 4, build_registry.py) reads entries.yaml directly and
does not depend on registry/derived/. These two outputs serve different audiences:
  - registry/derived/*.md   → humans browsing the repo on GitHub
  - www/docs/tonesu/registry/ → the public website (generated by build_registry.py)
