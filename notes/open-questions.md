...
- [ ] **`se` pronoun / `se` root collision.** Pronoun `se` (3rd person / other / they) shares form with root `se` (perception/sense). The collision is non-transparent: "other person" has no semantic relationship to perception. Candidate replacement: `ze` (clean CV, no root, no function-word assignment). Corpus impact minimal — fix is cheap now.
- [ ] **`wi` pronoun / `wi` root collision.** Pronoun `wi` (group / we / they) shares form with root `wi` (will/intention). Non-transparent. Additionally, `wi` is the candidate purpose-frame particle (`wi [action]` = "intending to") — if adopted, the pronoun and purpose-frame would be indistinguishable. Candidate replacement: `yu` (clean CV, no root, no function-word assignment). Resolve before purpose-clause structure is formalized.
