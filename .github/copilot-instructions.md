...
| VC | Admitted (one form) | Word-initial only; `el` (G028) = spoken/written-substitute for `—` (prosodic suspension); ~39 remaining slots reserved |
...
- `ke` pivot particle (`ke, [clause]` = implied denial + advancing claim; clause-initial + `,`; correct when prior denial is already contextually encoded and re-performing it adds no information; `ke!` = heated pivot; stacks as `ya, ke, [clause]`) · distinct from `no — [claim]` (which explicitly performs the denial — correct when denial is fresh)
...
|-----------|--------|---------|
| `'` | **normative — written** | Prosodic juncture marker. Marks the left boundary of a subcompound inside a longer chain. Right-binding: `A'BC` = A modifies [BC]. Minor phrase break in speech. Spoken form: `peld` (G021). **`peld` is a valid written substitute for `'`** wherever the apostrophe symbol is unavailable or a word-form is preferred. Spec: `spec/phonology.md §Prosodic Juncture Marker`. |
...
| `-` | **normative — metalinguistic/technical contexts** | Morpheme boundary mark. Marks root boundaries in a compound. Normative in metalinguistic, pedagogical, and math/science contexts — appears when morpheme structure is itself the subject of discourse or instruction. In ordinary Tonesu prose, compounds remain solid (`tofesuki` not `to-fe-su-ki`). Spoken form: `feld` (G018). **`feld` is a valid written substitute for `-`** wherever the ASCII hyphen is unavailable or a word-form is preferred. Spec: `spec/phonology.md §- — Morpheme Boundary Mark`. |
| `[]` | **normative — written** | Aside / commentary frame. In written Tonesu: wraps editorial or analytic annotation that does not alter truth conditions of surrounding text. Removal invariance: stripping all `[]` must leave the argument unchanged. Self-policing: may not upgrade evidential status or silently rewrite the core claim. Never appears in spec/grammar *analytic* notation (that role now belongs to `{}`). Legacy spec text may use `[]` analytically; those occurrences are grandfathered. Spoken forms: `zeld` (G022, open) and `zelds` (G023, close) — used in dictation and pedagogical contexts. **`zeld` and `zelds` are valid written substitutes** for `[` and `]` wherever the bracket symbols are unavailable or word-forms are preferred. Spec: `spec/phonology.md §[] — Aside / Commentary Frame`; `spec/grammar.md §Aside / Commentary Frame`. |
| `{}` | **normative — metalinguistic/technical/mathematical contexts** | Structural slot / scope bracket. Marks structural slot placeholders (`la-{agent}`), subordinate clause scope (`go {premise}, result`), set/group notation in math and science (`{X, Y, Z}`), and formal scope groupings. Spoken forms: `suld` (G019, open) and `sulds` (G020, close). **`suld` and `sulds` are valid written substitutes** for `{` and `}` wherever the brace symbols are unavailable or word-forms are preferred. Spec: `spec/phonology.md §{} — Structural Slot / Scope Bracket`. |
| `()` | **normative — written** | Evidential frame. Wraps a declarative clause to mark its content as reported, inferred, or unattributed — not directly asserted. Stackable with `~`: `~(clause)` = approximately/reportedly. Spoken forms: `vund` (G014, open bracket) and `vunds` (G015, close bracket). **`vund` and `vunds` are valid written substitutes** for `(` and `)` wherever the symbols are unavailable or word-forms are preferred. Spec: `spec/phonology.md §() — Evidential Frame`; `spec/grammar.md §Evidential Frame`. |
...
| `""` | **normative — written (probationary)** | Quotation / mention frame. Three uses: (1) direct speech — enclosed string is the literal utterance of the agent; (2) linguistic mention / use-mention distinction — the form is named or discussed as a token, not used; (3) titles and conceptual labels. Content inside `""` is structurally opaque to the enclosing clause. Distinguished from `()` (epistemic/unattributed) and `[]` (annotation). `()` inside `""` is disallowed. Spoken forms: `sild` (G016, open) and `silds` (G017, close). **`sild` and `silds` are valid written substitutes** for `"` and `"` wherever the symbols are unavailable or word-forms are preferred. Spec: `spec/phonology.md §"" — Quotation / Mention Frame`. Admitted March 2026; review after 20 corpus attestations. |
| `;` | **normative — written** | Sequential connector. A ; B = A, and then B (connection type not asserted — may be causal, incidental, adversative, or temporal). Distinct from `go` (which asserts a causal mechanism). The distinction is Hume's: constant conjunction (`;`) vs necessary connection (`go`). Constraints: single-clause premise only; left-to-right directionality only. Not valid where the deductive mechanism is the point (Cogito, modus ponens — use `go` there). Spoken form: `teld` (G027) in dictation; written-register distinction only. Admitted SCL-002 (March 2026). Spec: `spec/phonology.md §`;` — Sequential Connector`. |
| `—` | **normative — spoken + written** | Prosodic suspension mark. Holds a phrase, clause, or fragment in incompletion — suspended, not closed. No assertion about what follows. Distinct from `,` (closed boundary), `;` (directed sequence), `/` (structural pairing), `!` (force). May appear post-clause, mid-NP-list, after a bare fragment, or terminally. Spoken/written-substitute form: `el` (G028, VC tier — first VC-tier admission). **Author choice: `—` and `el` are equally normative.** Use `el` in verse, phonetic contexts, or wherever the em-dash glyph is unavailable or a word-form is preferred. Admitted EMD-002 (March 2026). Spec: `spec/phonology.md §— — Prosodic Suspension`. |
...
```
mkdocs.yml              ← PRODUCTION config (root). CI and build.py use this.
www/mkdocs.yml          ← DEPRECATED. Do not edit. Stale local-dev copy.
www/docs/               ← All website source files (Markdown, overrides, CSS).
site/                   ← Build output (gitignored). Generated by mkdocs build.
scripts/build.py        ← Full pipeline runner (steps 1–5).
.github/workflows/deploy.yml  ← CI: runs on push to main → deploys to GitHub Pages.
```

### Which mkdocs.yml to edit

**Always edit the ROOT `mkdocs.yml`.** It is the only config used by CI and `scripts/build.py`.
`www/mkdocs.yml` is deprecated and exists only as a historical artifact.
...
Flags: `--dry-run` (steps 2–3 won't write), `--skip N` (skip step N).

### Adding a new page to the website

1. Create the `.md` file under `www/docs/` in the appropriate subfolder.
2. Add a nav entry in the ROOT `mkdocs.yml` under the correct section.
3. Do **not** edit `www/mkdocs.yml`.
4. Run `python scripts/build.py` (or just `--skip 1 --skip 2 --skip 3` to test the site only).
5. Commit and push. CI deploys automatically on push to `main`.

### CI deployment

`.github/workflows/deploy.yml` runs on every push to `main`:
1. Runs `python scripts/build.py --skip 5` (steps 1–4 only).
2. Runs `mkdocs gh-deploy --force` (builds + deploys to GitHub Pages).
3. Adds a README to the `gh-pages` branch.

Live site: **https://tonesu.org/**
