...
Tonesu sentences from the corpus, with full breakdowns. The goal here is to show the grammar working in practice — not rules in isolation, but sentences doing real things.

*This section is being populated progressively. Check back as the corpus grows.*
...
Sentences covering:
- Causal chains (`go` / `du`)
- Evidential framing with `()`
- Topic frames with `:`
- Complex compounds with `'`
- Dialogue and multi-turn examples
