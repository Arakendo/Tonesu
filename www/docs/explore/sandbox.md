...
> **Notation:** Construction reasoning below uses analytic notation (hyphens show morpheme boundaries). The final compound in each case is shown in **written Tonesu** (no hyphens) as the named result.
...
wi-to-ki  →  entering the state of knowledge-seeking
          →  the moment of becoming curious  =  witoki
```
...
wi-to-li  →  person of knowledge-directed intention
          →  the curious one / the seeker  =  witoli
```
...
fa-ti  →  affective state anchored in time
        →  felt temporal state: memory mood, nostalgia-region  =  fati
```
...
fa-ti-di  →  affect + time + direction
          →  affect oriented toward time: the pull back  =  fatidi
...
go-no-ne  →  origin with no connection
          →  uncaused occurrence, random emergence  =  gonone
```
...
ne-vo-ka  →  relation between value and action
          ...
          →  rightness, justice as a condition (not an institution)  =  nevoka
```
...
fa-de  →  affect in decay
        →  felt loss, the degrading condition  =  fade
```

The compound is predicate-class: used as `lalali ne fade` ("X is in a state of felt loss"). The verbal form:

```
fa-de-ki  →  entering the state of felt loss
          →  to begin grieving, the onset of grief  =  fadeki
```
...
```
wineli  (wi-ne-li)  →  intention + relation + person
                   →  person who is in intentional relation
                   →  relational agent (not strong enough)
```

Try it differently. Trust is acting as if `loli wine lami` (X's intention is connected to mine):

```
no-to-wi  →  action from intention, without requiring knowledge first
          →  acting on intention without verification  =  notowi
```
...
```
no'towi  (no'to-wi)  = [no] • [to-wi]  = absence of [knowledge → intention] gate
