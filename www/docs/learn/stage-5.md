...
Every root in Tonesu operates at a default register — the ordinary, particular mode.
Scope prefixes shift that register without adding a new root. Five bare-vowel prefixes,
each with a distinct scope effect, each productive across the full root inventory.

---

## Cluster 1 — The prefix tier

Scope prefixes occupy the **V tier** — a bare vowel syllable, always word-initial.
Because Tonesu's parse invariants prohibit bare-vowel syllables mid-compound (every
internal syllable must begin with a consonant), a V-prefix is unambiguously
word-initial: its structural role is phonologically guaranteed.
...
!!! question "Exercise 1 — *Scholar or sage?*"
    ...
    <div class="tn-learn-mcq"
         data-answer="b"
         data-ok="Correct — a-to-li: the a- prefix shifts to-li from scholar to sage — a person engaged with knowing at the universal category level ✓"
         data-nok="to-li is the scholar — grounded in a domain. The scope prefix a- is what elevates the compound to the universal register."
         data-options='[{"id":"a","text":"`to-li` — knowledge-person: the disciplinary scholar"},{"id":"b","text":"`a-to-li` — universal-knowing-person: the sage, one engaged with knowing as such"}]'>
    </div>
...
??? success "Explanation"
    ...
    different epistemic orientation entirely.
...
- Not `to-no-fe` (omniscience, THO-001) — which asserts a *property* of knowing:
  that it has no end. `a-to ne no-fe` is a category claim about knowing *as such*.
- Not `to-su` (W030, organized knowledge system) — which is a specific instantiation
  of knowledge, a named compound.

```
a-to  helm  to-su
```
*Abstract knowing functions as an organized structure in practice — but is not
identical to it.*  (S505)
...
!!! question "Exercise 2 — *Category, system, or fact?*"
    ...
    <div class="tn-learn-mcq"
         data-answer="b"
         data-ok="Correct — a-to: knowing-in-general, the abstract category level. Not a system (to-su) and not a specific instance (i-to) ✓"
         data-nok="to-su is a specific organized system; i-to is a precise particular instance. The universal/abstract register needs the a- prefix."
         data-options='[{"id":"a","text":"`to-su` — organized knowledge system: a specific instantiation (W030)"},{"id":"b","text":"`a-to` — knowing-in-general: the abstract category, not any instance or system"},{"id":"c","text":"`i-to` — this precise fact: the particular/specific instance"}]'>
    </div>
...
!!! question "Exercise 3 — *Which prefix marks collective-as-unit?*"
    `___-li  ne  su` — the community, as a unified body (not a headcount), holds
    structure. Which scope prefix?

    <div class="tn-learn-picker"
         data-template="___-li  ne  su"
         data-answer="o"
         data-mode="form"
         data-ok="o — collective/distributed. o-li = community as a unified body (not pu-li, a count of individuals) ✓"
         data-nok="Check which prefix marks the collective-as-unit register: a- = universal, i- = particular, u- = interior, o- = collective, e- = in-process."
         data-items='[{"form":"a","gloss":"abstract / universal"},{"form":"i","gloss":"particular / precise"},{"form":"u","gloss":"interior / foundational"},{"form":"o","gloss":"collective / distributed"},{"form":"e","gloss":"transitional / in-process"}]'>
    </div>
...
The V-prefix scopes over the **entire following compound** under the right-branching
default:
...
V-prefix forms are **productive** — any root or compound can be scope-shifted.
They are not individually registered in the derived registry (no W-number); the
prefix transforms are systematic.
...
[Stage 6 — Epistemic discipline](stage-6.md) goes deeper into the epistemic layer:
