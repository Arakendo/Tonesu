...
| G014 | `kelm` | evidential frame opener (spoken) | spoken realization of `(` (evidential frame open); canonical written form is `(`; CVCC; forms the minimal pair `kelm` / `kelms` with G015. The `-lm` sonorant cluster parallels `helm`/`helms` — both are CVCC/CVCCS notation framing pairs. `k` initial consonant (sound symbolism: discrete boundary) reflects the frame-boundary function of `(`. `kelm` is a valid written substitute for `(` wherever the bracket symbol is unavailable. |
| G015 | `kelms` | evidential frame closer (spoken) | spoken realization of `)` (evidential frame close); canonical written form is `)`; CVCCS — produced by the bracket-pair convention (`kelm` + `s`; the `+s` coda marks the secondary/closing element, per the CVCCS admission rule). Forms the minimal pair `kelm` / `kelms` with G014. `kelms` is a valid written substitute for `)` wherever the bracket symbol is unavailable. |
