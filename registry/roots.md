...
| G031 | `ru-fe` | exclusive scope particle | clause-initial exclusive focus marker; `ru-fe, [clause]` = "only / solely / exclusively: [clause]." Compositional: `ru` (unity/singularity) + `fe` (boundary/limit) = "the singular bounding case" = the one point at which the proposition holds, and no other. Scope: `ru-fe` scopes the entire following proposition — agent, method/causal frame, identity predicate, or any argument structure. Syntactic position: clause-initial, followed by `,`, identical positioning to `ya` (G030). Stacking: `ya, ru-fe, [clause]` is admitted — pragmatic outer scope (`ya`) wraps semantic exclusive scope (`ru-fe`) wraps content. **Contrast with `la-X ne ru`** (cardinality predicate — "X is one/unified"; the Shema form) — that construction asserts a unity property; `ru-fe` marks exclusive restriction over the clause. **Do not use `la-X ru [predicate]`** — `ru` in post-agent position is syntactically ambiguous (EXC-001 S552 diagnostic). Written solid in Tonesu prose: `rufe,`. Four confirmed positions: agent-exclusive (S554), method-exclusive (S555), agent-narrative (S556), theological canonical with `helms` (S557). Admitted EXC-001 (March 2026). |
| G032 | `ke` | pivot particle | discourse pivot marker; `ke, [clause]` = "setting the prior exchange aside: [clause]." Signals that the speaker is moving forward with an affirmative position; the denial of whatever was asserted or assumed in the prior exchange is implied without re-performance. Syntax: clause-initial, followed by `,`; same positional slot as `ya` (G030) and `ru-fe` (G031). Stacking: `ya, ke, [clause]` = "attend: {pivot} [affirmative claim]" — pragmatic outer scope (`ya`) wraps discourse pivot (`ke`) wraps advancing content. **Contrast with `no — [claim]`**: `no —` performs the denial explicitly and is correct when the denial is fresh or adds new information; `ke` implies the denial and is correct when the denial has already been performed and its re-performance would add no new information (information-freshness rule). Three qualifying registers: heated philosophical debate (DEB-001 S545), formal clinical consultation (MED-001 S575), treaty negotiation (DIP-001 S585). `ke!` = heated pivot (denial implicit, force marked). Written solid: `ke,` before the advancing clause. CV: `k` = crisp/definite onset; `e` = salient mid-front vowel. Admitted DIP-001 (March 2026). |
