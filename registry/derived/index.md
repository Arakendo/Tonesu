# Derived Registry — Quick Reference Index

## Status: Normative

Full entry format and validation rules: [w001-w050.md](w001-w050.md).
New entries always go into **[w101-plus.md](w101-plus.md)** regardless of eventual W-number.

---

## Entry Counts (March 2026)

| File | Range | Entries |
|------|-------|---------|
| [w001-w050.md](w001-w050.md) | W000–W050 | 39 |
| [w051-w100.md](w051-w100.md) | W051–W100 | 50 |
| [w101-plus.md](w101-plus.md) | W101–W168 | 68 |

**Total: 155 W-series entries** — accepted: 1 · pending: 154

Status key: ✅ active/accepted · ⏳ pending · ⚠️ proposed · ❄️ cold · 🚫 retired

---

## A. Alphabetical Quick Reference

Sorted by form. Use this to look up a known compound. For concept-first lookup, see § B.

| Form | W# | Gloss | St | File |
|------|----|-------|----|------|
| `be-go-li-si` | W149 | creator-representation, image (tselem) | ⏳ | [w101-plus](w101-plus.md) |
| `de-ki'ma` | W162 | wine / fermented liquid | ⏳ | [w101-plus](w101-plus.md) |
| `de-su` | W119 | fault, wrong, moral failing | ⏳ | [w101-plus](w101-plus.md) |
| `de-vo` | W121 | evil, value-corruption quality | ⏳ | [w101-plus](w101-plus.md) |
| `de-be` | W035 | repair, restore | ⏳ | [w001-w050](w001-w050.md) |
| `di-ki-li` | W061 | navigator | ⏳ | [w051-w100](w051-w100.md) |
| `du-zo-su` | W142 | fruit, plant-result | ⏳ | [w101-plus](w101-plus.md) |
| `fa-de` | W094 | affect fading | ⏳ | [w051-w100](w051-w100.md) |
| `fa-de-ra` | W155 | suffering, affect-decrease-force | ⏳ | [w101-plus](w101-plus.md) |
| `fa-ra` | W154 | fear/awe, forceful affect | ⏳ | [w101-plus](w101-plus.md) |
| `fa-ra-be` | W123 | anger, hostile arousal | ⏳ | [w101-plus](w101-plus.md) |
| `fa-wi-de` | W127 | temptation, affect-will-toward-harm | ⏳ | [w101-plus](w101-plus.md) |
| `fa-wi-zi` | W136 | lust, affect-desire-to-couple | ⏳ | [w101-plus](w101-plus.md) |
| `fa-ki` | W093 | affect shift / change of register | ⏳ | [w051-w100](w051-w100.md) |
| `fa-no` | W095 | affect inactive (unresolved or numb) | ⏳ | [w051-w100](w051-w100.md) |
| `fa-re` | W096 | affect cycling / returning to baseline | ⏳ | [w051-w100](w051-w100.md) |
| `fe-no-ka` | W056 | taboo, forbidden boundary | ⏳ | [w051-w100](w051-w100.md) |
| `fe-si` | W024 | warning, boundary-condition signal | ⏳ | [w001-w050](w001-w050.md) |
| `fe-su` | W055 | ward, protective barrier | ⏳ | [w051-w100](w051-w100.md) |
| `fe-vo` | W065 | sacredness, set-apart quality | ⏳ | [w051-w100](w051-w100.md) |
| `go-ne` | W159 | self-originating relation / "I am who I am" | ⏳ | [w101-plus](w101-plus.md) |
| `ha-de` | W044 | cooling, thermal decrease | ⏳ | [w001-w050](w001-w050.md) |
| `ka-de-zo` | W128 | kill, deliberate ending of life | ⏳ | [w101-plus](w101-plus.md) |
| `ka-li-su` | W147 | governance, dominion, coordinated stewardship | ⏳ | [w101-plus](w101-plus.md) |
| `ka-no-de-su` | W120 | forgiveness, deliberate removal of fault | ⏳ | [w101-plus](w101-plus.md) |
| `ka-du-zo-su` | W161 | bread, deliberately crafted plant-product | ⏳ | [w101-plus](w101-plus.md) |
| `ka-pa-ki` | W160 | liberation act, deliberate place-motion | ⏳ | [w101-plus](w101-plus.md) |
| `ka-pa-ki'ti-re` | W167 | Passover festival, recurring liberation event | ⏳ | [w101-plus](w101-plus.md) |
| `ka-vo-si` | W168 | thanksgiving / blessing, deliberate value-signal | ⏳ | [w101-plus](w101-plus.md) |
| `ka-to-fe` | W122 | judge, deliberate epistemic bounding | ⏳ | [w101-plus](w101-plus.md) |
| `ka-mu` | W047 | tool, action instrument | ⏳ | [w001-w050](w001-w050.md) |
| `ka-ne-de` | W092 | deliberate bond dissolution | ⏳ | [w051-w100](w051-w100.md) |
| `ki-ne` | W152 | sandal, motion-bond (footwear) | ⏳ | [w101-plus](w101-plus.md) |
| `ki-pa` | W049 | corridor, passage | ⏳ | [w001-w050](w001-w050.md) |
| `ki-pa-mu` | W086 | vehicle, moving enclosed place | ⏳ | [w051-w100](w051-w100.md) |
| `ki'ma` | W114 | liquid (material state) | ⏳ | [w101-plus](w101-plus.md) |
| `ko-mu` | W052 | vessel, container artifact | ⏳ | [w051-w100](w051-w100.md) |
| `ko-pa` | W048 | room, enclosed space | ⏳ | [w001-w050](w001-w050.md) |
| `li-be` | W033 | child, person in growth phase | ⏳ | [w001-w050](w001-w050.md) |
| `li-pu-pa` | W036 | city, inhabited settlement | ⏳ | [w001-w050](w001-w050.md) |
| `li-su-li` | W001 | leader, coordinator, administrator | ⏳ | [w001-w050](w001-w050.md) |
| `lu-mu` | W139 | light-body, luminous celestial object | ⏳ | [w101-plus](w101-plus.md) |
| `lu-ti` | W140 | daytime, light-period | ⏳ | [w101-plus](w101-plus.md) |
| `ma-no-de` | W046 | medicine, treatment substance | ⏳ | [w001-w050](w001-w050.md) |
| `ma-ra` | W116 | plasma, ionized matter | ⏳ | [w101-plus](w101-plus.md) |
| `ma-zo-ki` | W146 | crawler, ground-moving creature | ⏳ | [w101-plus](w101-plus.md) |
| `mu-ka` | W022 | use / operate a tool or artifact | ⏳ | [w001-w050](w001-w050.md) |
| `ne-de` | W091 | bond dissolution (no assigned cause) | ⏳ | [w051-w100](w051-w100.md) |
| `ne-zi-re` | W131 | spouse, bonded coupling partner | ⏳ | [w101-plus](w101-plus.md) |
| `ne-fe` | W042 | dependency condition, requirement | ⏳ | [w001-w050](w001-w050.md) |
| `ne-ra` | W058 | resonance, energetic coupling state | ⏳ | [w051-w100](w051-w100.md) |
| `ne-ra-ki` | W059 | attune, enter resonance | ⏳ | [w051-w100](w051-w100.md) |
| `ne-su` | W031 | network, society, organization | ⏳ | [w001-w050](w001-w050.md) |
| `ne-to` | W084 | mutual acknowledgment, agreement | ⏳ | [w051-w100](w051-w100.md) |
| `ne-to-fe` | W165 | covenant, formally bounded agreement | ⏳ | [w101-plus](w101-plus.md) |
| `no-de-ma` | W129 | salt, preservative matter | ⏳ | [w101-plus](w101-plus.md) |
| `no-lu-ti` | W141 | nighttime, dark-period | ⏳ | [w101-plus](w101-plus.md) |
| `no-ne-fe` | W075 | absence of dependency / no link | ⏳ | [w051-w100](w051-w100.md) |
| `no-su'ma` | W115 | gas (material state) | ⏳ | [w101-plus](w101-plus.md) |
| `nu-se` | W102 | measurement, quantitative detection | ⏳ | [w101-plus](w101-plus.md) |
| `pa-be'ka-li-su` | W125 | kingdom of heaven, upper-space governance | ⏳ | [w101-plus](w101-plus.md) |
| `pa-fe` | W138 | vault, firmament, sky-dome | ⏳ | [w101-plus](w101-plus.md) |
| `pa-ki` | W087 | adrift, uncontrolled spatial motion | ⏳ | [w051-w100](w051-w100.md) |
| `pa-ki` / `pa-ki-li` | W156 | liberation / liberator, place-motion (⚠️ form-shares W087 — pending resolution) | ⚠️ | [w101-plus](w101-plus.md) |
| `pa-mi` | W110 | here, speaker's location | ⏳ | [w101-plus](w101-plus.md) |
| `pa-nu` | W050 | size, spatial magnitude | ⏳ | [w001-w050](w001-w050.md) |
| `pa-ra` | W053 | energetic field, spatial force distribution | ⏳ | [w051-w100](w051-w100.md) |
| `pa-su` | W157 | mountain, structured place | ⏳ | [w101-plus](w101-plus.md) |
| `pa-vo-fe` | W153 | holy ground, value-bounded place | ⏳ | [w101-plus](w101-plus.md) |
| `pa-wi` | W088 | destination, intentional target place | ⏳ | [w051-w100](w051-w100.md) |
| `pa-wi-ka-su` | W066 | shrine, temple | ⏳ | [w051-w100](w051-w100.md) |
| `pa-zo-li` | W135 | the world, inhabited realm of human persons | ⏳ | [w101-plus](w101-plus.md) |
| `ra-ki` | W038 | storm, kinetic-force discharge | ⏳ | [w001-w050](w001-w050.md) |
| `ra-ki-mu` | W012 | engine, motor, generator | ⏳ | [w001-w050](w001-w050.md) |
| `ra-lu-ki` | W151 | fire, energy-light-process | ⏳ | [w101-plus](w101-plus.md) |
| `ra-ma` | W082 | fuel, energy substrate | ⏳ | [w051-w100](w051-w100.md) |
| `ra-vo` | W071 | signal intensity, energy quality | ⏳ | [w051-w100](w051-w100.md) |
| `re-ka-ne-li` | W134 | peacemaker, habitual bond-creator | ⏳ | [w101-plus](w101-plus.md) |
| `se-ka` | W034 | examination, deliberate inspection | ⏳ | [w001-w050](w001-w050.md) |
| `se-lu` | W130 | eye, visual perception organ | ⏳ | [w101-plus](w101-plus.md) |
| `se-no-to` | W090 | percept without cognitive model (unresolved signal) | ⏳ | [w051-w100](w051-w100.md) |
| `se-to` | W062 | perceptual insight, vision | ⏳ | [w051-w100](w051-w100.md) |
| `si-de` | W098 | past signal, signal of record | ⏳ | [w051-w100](w051-w100.md) |
| `si-fe` | W070 | signal threshold, detection boundary | ⏳ | [w051-w100](w051-w100.md) |
| `si-ki` | W023 | transmit a signal | ⏳ | [w001-w050](w001-w050.md) |
| `si-ki-li` | W158 | messenger/angel, signal-motion-agent | ⏳ | [w101-plus](w101-plus.md) |
| `si-ko-mu` | W067 | individual document, encoded record | ⏳ | [w051-w100](w051-w100.md) |
| `si-mu` | W039 | relay device, signal transceiver | ⏳ | [w001-w050](w001-w050.md) |
| `si-no-vo-li` | W126 | hypocrite, person of valueless signal | ⏳ | [w101-plus](w101-plus.md) |
| `si-ne-ki` | W021 | signal-transmission process | ⏳ | [w001-w050](w001-w050.md) |
| `su-fa` | W133 | purity of heart, structured affect | ⏳ | [w101-plus](w101-plus.md) |
| `su-mu-li` | W002 | engineer, architect | ⏳ | [w001-w050](w001-w050.md) |
| `su-ru` | W085 | integrated whole, unified structure | ⏳ | [w051-w100](w051-w100.md) |
| `su-ti` | W101 | current state, instantaneous configuration | ⏳ | [w101-plus](w101-plus.md) |
| `su'ma` | W113 | solid (material state) | ⏳ | [w101-plus](w101-plus.md) |
| `ti-be` | W040 | morning; proximate future | ⏳ | [w001-w050](w001-w050.md) |
| `ti-de` | W041 | past time; yesterday | ⏳ | [w001-w050](w001-w050.md) |
| `ti-fe` | W037 | deadline, temporal boundary | ⏳ | [w001-w050](w001-w050.md) |
| `ti-mi` | W109 | now, present moment (deictic) | ⏳ | [w101-plus](w101-plus.md) |
| `ti-mu` | W057 | relic, ancient artifact | ⏳ | [w051-w100](w051-w100.md) |
| `ti-re` | W103 | recurring time, scheduled cycle | ⏳ | [w101-plus](w101-plus.md) |
| `to-fe` | W028 | epistemic boundary (the-line between knowledge states) | ⏳ | [w001-w050](w001-w050.md) |
| `to-fe-ka` | W029 | deliberate epistemic mislabeling | ⏳ | [w001-w050](w001-w050.md) |
| `to-fe-li` | W032 | epistemic guardian/adjudicator | ⏳ | [w001-w050](w001-w050.md) |
| `to-fe-su` | W072 | epistemic standards body/code | ⏳ | [w051-w100](w051-w100.md) |
| `to-fe-su-ki` | W097 | epistemic ruling crossing to public force | ⏳ | [w051-w100](w051-w100.md) |
| `to-go` | W089 | counterfactual frame marker | ⏳ | [w051-w100](w051-w100.md) |
| `to-ki` | W020 | computation, reasoning process | ⏳ | [w001-w050](w001-w050.md) |
| `to-ki-mu` | W011 | computer, calculator | ⏳ | [w001-w050](w001-w050.md) |
| `to-ko` | W027 | memory, stored knowledge | ⏳ | [w001-w050](w001-w050.md) |
| `to-ko-re` | W166 | remembrance, memory-return | ⏳ | [w101-plus](w101-plus.md) |
| `to-li` | W003 | scholar, expert, knower | ⏳ | [w001-w050](w001-w050.md) |
| `to-ne-su` | W000 | truth; the language itself | ✅ | [w001-w050](w001-w050.md) |
| `to-re-su` | W064 | canonical doctrine, scripture | ⏳ | [w051-w100](w051-w100.md) |
| `to-ru` | W083 | consensus, shared conceptual identity | ⏳ | [w051-w100](w051-w100.md) |
| `to-si` | W026 | query; proposition / thought-signal | ⏳ | [w001-w050](w001-w050.md) |
| `to-su` | W030 | organized knowledge, theory, model | ⏳ | [w001-w050](w001-w050.md) |
| `to-su-ki` | W025 | comprehend, enter organized knowledge | ⏳ | [w001-w050](w001-w050.md) |
| `to-su-mu` | W010 | database, library, knowledge-store | ⏳ | [w001-w050](w001-w050.md) |
| `vo-mu` | W137 | garment, clothing, value-artifact worn on body | ⏳ | [w101-plus](w101-plus.md) |
| `vo-ne` | W118 | righteousness, right relational standing | ⏳ | [w101-plus](w101-plus.md) |
| `wi-de-li` | W124 | adversary, enemy, opposing-will person | ⏳ | [w101-plus](w101-plus.md) |
| `wi-fe` | W100 | rule, prohibition, policy-imposed limit | ⏳ | [w051-w100](w051-w100.md) |
| `wi-ka-su` | W054 | ritual, structured intentional practice | ⏳ | [w051-w100](w051-w100.md) |
| `wi-no-ra` | W132 | meekness, will-without-force | ⏳ | [w101-plus](w101-plus.md) |
| `wi-re` | W099 | feedback loop, iterative goal-cycle | ⏳ | [w051-w100](w051-w100.md) |
| `wi-re-su` | W074 | operational protocol, standing procedure | ⏳ | [w051-w100](w051-w100.md) |
| `wi-si` | W063 | prayer, will-signal | ⏳ | [w051-w100](w051-w100.md) |
| `wi-to` | W045 | plan (single-instance purposive artifact) | ⏳ | [w001-w050](w001-w050.md) |
| `wi-to-su` | W073 | design specification, formal plan | ⏳ | [w051-w100](w051-w100.md) |
| `wi-zo` | W145 | wild animal, autonomous organism | ⏳ | [w101-plus](w101-plus.md) |
| `zi-ka` | W105 | transaction, intentional exchange event | ⏳ | [w101-plus](w101-plus.md) |
| `zi-ra` | W104 | physical interaction, mutual energy transfer | ⏳ | [w101-plus](w101-plus.md) |
| `zi-zo` | W106 | biological coupling event | ⏳ | [w101-plus](w101-plus.md) |
| `zi-zo-du` | W108 | du-role in biological coupling | ⏳ | [w101-plus](w101-plus.md) |
| `zi-zo-go` | W107 | go-role in biological coupling | ⏳ | [w101-plus](w101-plus.md) |
| `zo-de` | W051 | fatigue, physical tiredness | ⏳ | [w051-w100](w051-w100.md) |
| `zo-li` | W148 | human person, living-person class | ⏳ | [w101-plus](w101-plus.md) |
| `zo-ki'ma` | W164 | blood, living liquid | ⏳ | [w101-plus](w101-plus.md) |
| `zo-ma` | W163 | body, physical matter of a living being | ⏳ | [w101-plus](w101-plus.md) |
| `zo-ne` | W076 | biological kinship bond | ⏳ | [w051-w100](w051-w100.md) |
| `zo-ne-du` | W078 | offspring, child (kinship) | ⏳ | [w051-w100](w051-w100.md) |
| `zo-ne-du-re` | W081 | descendant | ⏳ | [w051-w100](w051-w100.md) |
| `zo-ne-go` | W077 | parent | ⏳ | [w051-w100](w051-w100.md) |
| `zo-ne-go-re` | W080 | ancestor | ⏳ | [w051-w100](w051-w100.md) |
| `zo-ne-ru` | W079 | sibling | ⏳ | [w051-w100](w051-w100.md) |
| `zo-no-ki` | W043 | rest, sleep | ⏳ | [w001-w050](w001-w050.md) |
| `zo-ra` | W117 | life-energy, animating vitality | ⏳ | [w101-plus](w101-plus.md) |
| `zo-ra-ma` | W144 | food, organism-energy-substrate | ⏳ | [w101-plus](w101-plus.md) |
| `zo-se-ki` | W060 | enter trance, heightened bodyperception state | ⏳ | [w051-w100](w051-w100.md) |
| `zo-si` | W069 | spirit, disembodied living agent | ⏳ | [w051-w100](w051-w100.md) |
| `zo-su-be` | W143 | seed, plant-growth-product | ⏳ | [w101-plus](w101-plus.md) |
| `zo-su-ka-li` | W150 | shepherd, structured-organism-tending-agent | ⏳ | [w101-plus](w101-plus.md) |
| `zo-to` | W068 | soul, organism identity-pattern | ⏳ | [w051-w100](w051-w100.md) |

*Retired entries (W111 `de-ti`, W112 `be-ti`) excluded — root-order error, March 2026.*

---

## B. Semantic Groupings

Concept-first lookup: find the compound you need by domain.

**Person / Agent**
`li-su-li` W001 leader · `su-mu-li` W002 engineer · `to-li` W003 scholar · `li-be` W033 child (developmental) · `to-fe-li` W032 epistemic guardian · `di-ki-li` W061 navigator · `si-no-vo-li` W126 hypocrite · `wi-de-li` W124 adversary/enemy · `zo-li` W148 human person · `si-ki-li` W158 messenger/envoy · `zo-su-ka-li` W150 shepherd · `pa-ki-li` W156 liberator

**Kinship**
`zo-ne` W076 kinship bond · `zo-ne-go` W077 parent · `zo-ne-du` W078 offspring · `zo-ne-ru` W079 sibling · `zo-ne-go-re` W080 ancestor · `zo-ne-du-re` W081 descendant · `ne-zi-re` W131 spouse

**Place / Space**
`li-pu-pa` W036 city · `ko-pa` W048 room · `ki-pa` W049 corridor · `pa-ra` W053 energetic field · `pa-wi-ka-su` W066 shrine · `pa-wi` W088 destination · `pa-mi` W110 here (deictic) · `pa-be'ka-li-su` W125 kingdom of heaven · `pa-fe` W138 vault/sky-dome · `pa-zo-li` W135 the world / inhabited realm · `pa-su` W157 mountain · `pa-vo-fe` W153 holy ground

**Artifact / Device**
`to-su-mu` W010 database · `to-ki-mu` W011 computer · `ra-ki-mu` W012 engine · `ka-mu` W047 tool · `si-mu` W039 relay device · `ko-mu` W052 container · `si-ko-mu` W067 document · `ki-pa-mu` W086 vehicle · `ma-no-de` W046 medicine · `wi-to` W045 plan · `vo-mu` W137 clothing/garment · `ki-ne` W152 sandal · `ka-du-zo-su` W161 bread

**Time**
`ti-de` W041 past · `ti-be` W040 morning/upcoming · `ti-mi` W109 now · `ti-re` W103 recurring cycle · `ti-fe` W037 deadline · `ti-mu` W057 relic (old artifact) · `lu-ti` W140 daytime · `no-lu-ti` W141 nighttime

**Knowledge / Epistemic**
`to-ne-su` W000 truth · `to-su` W030 theory/model · `to-ki` W020 reasoning · `to-ko` W027 memory · `to-ko-re` W166 remembrance · `to-si` W026 query/proposition · `to-su-ki` W025 comprehend · `to-su-mu` W010 database · `to-ru` W083 consensus · `to-fe` W028 epistemic boundary · `to-fe-ka` W029 mislabeling · `to-fe-li` W032 adjudicator · `to-fe-su` W072 standards body · `to-fe-su-ki` W097 ruling → public · `to-go` W089 counterfactual frame · `to-re-su` W064 scripture · `ka-to-fe` W122 act of judging

**Signal / Communication**
`si-ki` W023 transmit · `si-mu` W039 relay · `si-fe` W070 threshold · `si-de` W098 record · `si-ko-mu` W067 document · `ra-vo` W071 signal intensity · `fe-si` W024 warning · `mu-ka` W022 operate/use

**Affect / Emotional substrate**
`fa-ki` W093 shift · `fa-de` W094 fading · `fa-no` W095 inactive · `fa-re` W096 cycling · `se-no-to` W090 unresolved percept · `fa-ra-be` W123 anger · `fa-wi-de` W127 temptation · `su-fa` W133 purity of heart · `wi-no-ra` W132 meekness · `fa-wi-zi` W136 lust · `fa-ra` W154 fear/awe · `fa-de-ra` W155 suffering

**Ritual / Sacred**
`wi-ka-su` W054 ritual · `fe-vo` W065 sacredness · `pa-wi-ka-su` W066 shrine · `wi-si` W063 prayer · `ka-vo-si` W168 thanksgiving · `to-re-su` W064 scripture · `zo-to` W068 soul · `zo-si` W069 spirit · `fe-no-ka` W056 taboo · `se-to` W062 vision/insight · `zo-ra` W117 life-energy · `vo-ne` W118 righteousness · `pa-be'ka-li-su` W125 kingdom of heaven · `be-go-li-si` W149 creator-representation/image · `ka-li-su` W147 governance/dominion · `pa-vo-fe` W153 holy ground · `go-ne` W159 self-originating relation · `ka-pa-ki'ti-re` W167 Passover festival

**Social / Institutional**
`ne-su` W031 network/society · `ne-to` W084 agreement · `ne-to-fe` W165 covenant · `to-fe-su` W072 standards body · `wi-fe` W100 rule/prohibition · `wi-re-su` W074 protocol · `wi-to-su` W073 specification · `wi-re` W099 feedback loop · `li-pu-pa` W036 city · `si-no-vo-li` W126 hypocrite · `wi-de-li` W124 adversary · `ne-zi-re` W131 spouse · `ka-no-de-su` W120 forgiveness · `de-su` W119 fault/wrong · `de-vo` W121 evil · `re-ka-ne-li` W134 peacemaker · `ka-li-su` W147 governance/dominion · `pa-vo-fe` W153 holy ground · `go-ne` W159 self-originating relation

**Organism / Biology**
`zo-de` W051 fatigue · `zo-no-ki` W043 sleep · `zo-se-ki` W060 trance · `zi-zo` W106 coupling event · `zi-zo-go` W107 go-role · `zi-zo-du` W108 du-role · `zo-to` W068 soul · `zo-si` W069 spirit · `zo-ra` W117 life-energy · `ka-de-zo` W128 to kill · `se-lu` W130 eye · `zo-li` W148 human person · `wi-zo` W145 wild animal · `ma-zo-ki` W146 crawler · `du-zo-su` W142 fruit · `zo-su-be` W143 seed · `zo-ra-ma` W144 food · `zo-su-ka-li` W150 shepherd · `zo-ma` W163 body · `zo-ki'ma` W164 blood · `de-ki'ma` W162 wine

**Energy / Matter / Physics**
`ra-ki` W038 storm · `ra-ki-mu` W012 engine · `ra-ma` W082 fuel · `ra-vo` W071 intensity · `pa-ra` W053 field · `ne-ra` W058 resonance state · `ne-ra-ki` W059 attune · `zi-ra` W104 physical interaction · `ha-de` W044 cooling · `su'ma` W113 solid · `ki'ma` W114 liquid · `no-su'ma` W115 gas · `ma-ra` W116 plasma · `nu-se` W102 measurement · `lu-mu` W139 light-body · `ra-lu-ki` W151 fire

**Relation / State / Quality**
`ne-fe` W042 dependency · `no-ne-fe` W075 no-link · `ne-de` W091 bond dissolving · `ka-ne-de` W092 deliberate dissolution · `su-ru` W085 unified whole · `pa-ki` W087 adrift · `pa-nu` W050 size · `de-be` W035 repair · `no-de-ma` W129 preservative/salt · `su-fa` W133 pure-in-heart

---

## C. Root Families

Quick lookup by lead primitive root.

| Lead root | Key entries (W#) |
|-----------|-----------------|
| `de` | de-be W035, de-su W119, de-vo W121, de-ki'ma W162 |
| `be` | be-go-li-si W149 |
| `di` | di-ki-li W061 |
| `du` | du-zo-su W142 |
| `fa` | fa-ki W093, fa-de W094, fa-no W095, fa-re W096, fa-ra W154, fa-de-ra W155, fa-ra-be W123, fa-wi-de W127, fa-wi-zi W136 |
| `fe` | fe-si W024, fe-su W055, fe-no-ka W056, fe-vo W065 |
| `go` | go-ne W159 |
| `ha` | ha-de W044 |
| `ka` | ka-mu W047, ka-ne-de W092, ka-to-fe W122, ka-de-zo W128, ka-no-de-su W120, ka-pa-ki W160, ka-pa-ki'ti-re W167, ka-vo-si W168, ka-du-zo-su W161, ka-li-su W147 |
| `ki` | ki-ne W152, ki-pa W049, ki-pa-mu W086, ki'ma W114 |
| `lu` | lu-mu W139, lu-ti W140 |
| `ko` | ko-mu W052, ko-pa W048 |
| `li` | li-su-li W001, li-be W033, li-pu-pa W036 |
| `ma` | ma-no-de W046, ma-ra W116, ma-zo-ki W146 |
| `mu` | mu-ka W022 |
| `ne` | ne-fe W042, ne-su W031, ne-ra W058, ne-ra-ki W059, ne-to W084, ne-to-fe W165, ne-de W091, ne-zi-re W131 |
| `no` | no-ne-fe W075, no-su'ma W115, no-de-ma W129, no-lu-ti W141 |
| `nu` | nu-se W102 |
| `re` | re-ka-ne-li W134 |
| `pa` | pa-nu W050, pa-ra W053, pa-su W157, pa-vo-fe W153, pa-wi W088, pa-wi-ka-su W066, pa-ki W087, pa-ki-li W156, pa-mi W110, pa-be'ka-li-su W125, pa-fe W138, pa-zo-li W135 |
| `ra` | ra-ki-mu W012, ra-ki W038, ra-lu-ki W151, ra-vo W071, ra-ma W082 |
| `se` | se-ka W034, se-to W062, se-no-to W090, se-lu W130 |
| `si` | si-ki W023, si-ki-li W158, si-mu W039, si-fe W070, si-ko-mu W067, si-de W098, si-ne-ki W021, si-no-vo-li W126 |
| `su` | su-mu-li W002, su-ru W085, su-ti W101, su'ma W113, su-fa W133 |
| `ti` | ti-fe W037, ti-be W040, ti-de W041, ti-mu W057, ti-re W103, ti-mi W109 |
| `to` | to-ne-su W000, to-li W003, to-ki-mu W011, to-ki W020, to-si W026, to-ko W027, to-ko-re W166, to-fe W028, to-fe-ka W029, to-su W030, to-fe-li W032, to-su-ki W025, to-su-mu W010, to-re-su W064, to-fe-su W072, to-go W089, to-ru W083, to-fe-su-ki W097 |
| `vo` | vo-ne W118, vo-mu W137 |
| `wi` | wi-to W045, wi-ka-su W054, wi-si W063, wi-to-su W073, wi-re-su W074, wi-re W099, wi-fe W100, wi-no-ra W132, wi-de-li W124, wi-zo W145 |
| `zi` | zi-ra W104, zi-ka W105, zi-zo W106, zi-zo-go W107, zi-zo-du W108 |
| `zo` | zo-no-ki W043, zo-de W051, zo-se-ki W060, zo-to W068, zo-si W069, zo-ne W076, zo-ne-go W077, zo-ne-du W078, zo-ne-ru W079, zo-ne-go-re W080, zo-ne-du-re W081, zo-ra W117, zo-li W148, zo-ra-ma W144, zo-su-be W143, zo-su-ka-li W150, zo-ma W163, zo-ki'ma W164 |
